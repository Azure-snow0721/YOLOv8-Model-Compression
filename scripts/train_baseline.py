...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train YOLOv8n baseline on VOC2012.")
...
def main() -> None:
    ...
    model = YOLO(cfg["baseline_model"])
    if args.resume:
        model = YOLO(str(latest_checkpoint("baseline_yolov8n", "last.pt")))
        model.train(resume=True)
    else:
        model.train(
            data=str(data_yaml),
            imgsz=args.imgsz or int(cfg["imgsz_train"]),
            epochs=args.epochs or int(cfg["epochs_baseline"]),
            batch=int(cfg["batch"]),
